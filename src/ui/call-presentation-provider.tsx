...
export function CallPresentationProvider({
  children,
  session,
  ...
  const [mode, setMode] = useState<CallPresentationMode>(
    session == null ? "idle" : "fullscreen",
  );
  ...
  const theme = useMemo(
    () => ({ ...defaultCallPresentationTheme, ...themeOverrides }),
    [themeOverrides],
  );

  const updateMode = useCallback((nextMode: CallPresentationMode) => {
  ...
  const prepareSystemPictureInPicture = useCallback(
    async (activeSession: HybridCallSession, force = false) => {
  ...
  const handlePictureInPictureEvent = useCallback(
    (event: PictureInPictureEvent) => {
      switch (event.type) {
        ...
        case "failedToStart":
          ...
          reportError(
            new Error(event.error ?? "Unable to start Picture in Picture"),
          );
          updateMode(
        ...
        case "restoreRequested":
          ...
          afterNextLayout(() => {
            ...
            void CallManager.completePictureInPictureRestore(true).catch(
              reportError,
            );
          });
          break;
  ...
  useEffect(() => {
    if (mode !== "inAppMini" || !isAndroid || session == null) return;
    afterNextLayout(() => {
      const activeSession = sessionRef.current;
  ...
  const restore = useCallback(async () => {
    ...
    if (modeRef.current !== "systemPip") return;
  ...
  return (
    <CallPresentationContext value={value}>{children}</CallPresentationContext>
  );
}
