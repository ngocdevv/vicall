...
import type { CallOverlayHostProps } from "./call-presentation.types";
...
export function CallOverlayHost({
  ...
  const resolvedMiniWidth = Math.min(
    miniWidth,
    Math.max(120, window.width - edgeInset * 2),
  );
  const resolvedMiniHeight = Math.min(
    miniHeight,
    Math.max(88, window.height * 0.32),
  );
  ...
  const rightBound = Math.max(
    leftBound,
    window.width - edgeInset - resolvedMiniWidth,
  );
  ...
  useEffect(() => {
    controlsOpacity.value = withTiming(
      mode === "fullscreen" && controlsVisible ? 1 : 0,
      { duration: 180 },
    );
  ...
  useEffect(() => {
    ...
    if (mode === "inAppMini") {
      ...
      if (shouldInitialize) {
        ...
        surfaceY.value = withSpring(
          clampCallOverlay(surfaceY.value, topBound, bottomBound),
          SPRING,
        );
      }
  ...
  useEffect(() => {
    localX.value = withSpring(
      clampCallOverlay(
        localX.value,
        leftBound,
        window.width - edgeInset - localWidth,
      ),
      SPRING,
    );
    localY.value = withSpring(
      clampCallOverlay(localY.value, localMinY, localMaxY),
      SPRING,
    );
  ...
  const surfacePan = Gesture.Pan()
    ...
    .onUpdate((event) => {
      if (modeCode.value === 0) {
        ...
        const progress = clampCallOverlay(
          distance / (window.height * 0.72),
          0,
          1,
        );
        surfaceX.value = interpolate(progress, [0, 1], [0, rightBound]);
        surfaceY.value = interpolate(progress, [0, 1], [0, bottomBound]);
        surfaceWidth.value = interpolate(
          progress,
          [0, 1],
          [window.width, resolvedMiniWidth],
        );
        surfaceHeight.value = interpolate(
          progress,
          [0, 1],
          [window.height, resolvedMiniHeight],
        );
    ...
    .onEnd((event) => {
      if (modeCode.value === 0) {
        const shouldMinimize = shouldMinimizeCall(
          event.translationY,
          event.velocityY,
          window.height,
        );
        if (shouldMinimize) {
          ...
          surfaceRadius.value = withTiming(
            22,
            { duration: 160 },
            (finished) => {
              if (finished) runOnJS(requestMinimize)();
            },
          );
        } else {
          surfaceX.value = withSpring(0, SPRING);
      ...
      if (modeCode.value !== 1) return;
      const release = resolveCallOverlayRelease(
        surfaceX.value,
        surfaceY.value,
        resolvedMiniWidth,
        resolvedMiniHeight,
  ...
  const localPan = Gesture.Pan()
    ...
    .onUpdate((event) => {
      localX.value = clampCallOverlay(
        localStartX.value + event.translationX,
        leftBound,
        localMaxX,
      );
      localY.value = clampCallOverlay(
        localStartY.value + event.translationY,
        localMinY,
        localMaxY,
      );
    })
    .onEnd(() => {
      localX.value = withSpring(
        localX.value + localWidth / 2 < window.width / 2
          ? leftBound
          : localMaxX,
        SPRING,
      );
      localY.value = withSpring(
        localY.value + localHeight / 2 < window.height / 2
          ? localMinY
          : localMaxY,
        SPRING,
      );
  ...
  if (session == null || mode === "idle") return null;

  const fullscreenLike =
    mode === "fullscreen" || mode === "minimizing" || mode === "restoring";
  const pointerEvents =
    mode === "systemPip" ? "none" : fullscreenLike ? "auto" : "box-none";
  ...
  return (
    <View
      ...
      style={{
        backgroundColor:
          fullscreenLike || (mode === "systemPip" && !isIOS)
    ...
    >
      ...
      <GestureDetector gesture={surfaceGesture}>
        ...
        >
          ...
          >
            <Text style={{ color: theme.contentColor, fontSize: 22 }}>‹</Text>
          </Animated.View>
          <Animated.View
            pointerEvents="none"
            style={[
      ...
      {session.localVideo != null && (
        <GestureDetector gesture={localPan}>
          <Animated.View
            pointerEvents={mode === "fullscreen" ? "auto" : "none"}
            style={[
              {
                backgroundColor: theme.backgroundColor,
                borderColor: theme.miniBorderColor,
          ...
          >
            <View style={{ flex: 1 }}>{session.localVideo}</View>
            <MuteBadge muted={session.localMuted} side="right" theme={theme} />
          </Animated.View>
        </GestureDetector>
      )}
      ...
      <Animated.View
        pointerEvents={
          mode === "fullscreen" && controlsVisible ? "box-none" : "none"
        }
        style={[
